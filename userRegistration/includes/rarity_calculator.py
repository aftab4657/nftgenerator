...
class RarityCalculator:
    ...
    percentages = []
    rarities = []
    def __init__(self, nfts,colors, rarities, percentages, dynamicPath):
        self.colors = colors
        ...
        for nfts in nfts.nfts:
            for nft in nfts:
                ...
                for i in range (len(nft.attributes)):
                    print("keyss", nft.attributes[i][1])
    ...
    def PlotPercentages(self, percentagesList, rarityList, nfts):
        ...
        for i in range (len(nfts.attributes)):
            ...
            plt.savefig(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamicPath +'/rarity/plots/' + nfts.attributes[i]+'.png'))
            print('Done.')
    ...
    def CreateRaritiesJsonReadable(self, nfts, rarityList, percentagesList, stockList):
        ...
        with open(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamicPath +'/rarity/raritiesToReactMap.json'), 'w') as jsonFile:
    ...
    def CreateNFTRanking(self, nfts, percentageList, stockList):
        ...
        with open(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamicPath +'/rarity/nftsRanking.json'), 'w') as jsonFile:
