...
class NftCreator:
    ...
    testRarities = False
    totalMetadata = []
    dynamic_folder_input = ""
    dynamic_folder_output = ""
    totalDNA = []
    def __init__(self, numberNFTs, folder_paths, testRarities, randomizeOutput, collection_input, collection_output):
        self.testRarities = testRarities
        self.dynamic_folder_input = collection_input
        self.dynamic_folder_output = collection_output
        ...
        for i in range(len(self.nftsQuantity)):
            ...
            if not self.testRarities:
                os.mkdir(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output + "/nfts/" +folder_paths[i]))
    ...
    def CreateOutputFile(self):
        if not self.testRarities:
            ...
            shutil.rmtree(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output))
            os.mkdir(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output))
            os.mkdir(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output + "/nfts"))
        else:
            print('Cleaning output/rarity folder...', end = ' ', flush = True)
            try:
                shutil.rmtree(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output + "/rarity"))
            except Exception as e:
                print(e)
                pass
        os.mkdir(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output + "/rarity"))
        os.mkdir(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output + "/rarity/plots"))
        print('Done.')

    #Returns the json template to use on our nfts
    def GetJsonTemplate(self):
        with open(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_input + '/template.json')) as template:
    ...
    def GetAttributesList(self):
        ...
        orderedLayersPath = natsorted(os.listdir(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_input + '/assets')))
        if len(orderedLayersPath) <= 1:
            print('ERROR. You need at least 2 differents attributes.')
            raise Exception("ERROR. You need at least 2 differents attributes.")
    ...
    def GetItemsList(self, attributesPath,attributes):
        ...
        if maxPossibilities <= 1:
            ...
            raise Exception('ERROR. You just have', maxPossibilities, 'possibilites, create more assets')
    ...
    def GetItemsPerAttribute(self, attributePath, attribute):
        ...
        files = natsorted(os.listdir(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_input + '/assets/' + attributePath)))
        itemTombola = [0]
        item = []
        itemPath = files
        for file in files:
        ...
        print('Done.  GetItemsPerAttribute(self, attributePath, attribute)')
    ...
    def CreateNfts(self, nftTotalQuantity, jsonTemplate, attributes, orderedLayersPath, items, itemsPath, itemsTombola, folder_path):
        ...
        for dna in nftsThisRun:
            ...
            nft = Nft(nftsCounterThisRun, nftMetadata['name'], rawAttributes, filteredAttributes,nftMetadata, nftPaths, orderedLayersPath, attributes_count, folder_path, self.dynamic_folder_input, self.dynamic_folder_output)
    ...
    def CreateExtraJsonFiles(self, i, folder_path):
        ...
        with open(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output +'/nfts/'+ folder_path+'_metadata.json'), 'w') as jsonFile:
            json.dump(self.totalMetadata[i], jsonFile, indent = 4)

        with open(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output +'/nfts/'+ folder_path+'_dna.json'), 'w') as jsonFile:
    ...
    def CreateTotalNFTInfo(self):
        ...
        with open(Path(os.path.abspath(os.path.dirname(__file__) + '/../../') + self.dynamic_folder_output +'/nfts.json'), 'w') as jsonFile:
