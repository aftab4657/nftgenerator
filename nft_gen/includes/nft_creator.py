...
class NftCreator:
    ...
    totalMetadata = []
    totalDNA = []
    def __init__(self, numberNFTs, folder_paths, testRarities, randomizeOutput, dir_path):
        ...
        for i in range(len(self.nftsQuantity)):
            ...
            if not self.testRarities:
                os.mkdir(self.dir_path + '/output/nfts/'+folder_paths[i])
    ...
    def CreateOutputFile(self):
        if not self.testRarities:
            ...
            shutil.rmtree(self.dir_path + '/output')
            os.mkdir(self.dir_path + '/output')
            os.mkdir(self.dir_path + '/output/nfts')
        else:
            print('Cleaning output/rarity folder...', end = ' ', flush = True)
            shutil.rmtree(self.dir_path + '/output/rarity')
        os.mkdir(self.dir_path + '/output/rarity')
        os.mkdir(self.dir_path + '/output/rarity/plots')
        print('Done.')

    #Returns the json template to use on our nfts
    def GetJsonTemplate(self):
        with open(self.dir_path + '/input/template.json') as template:
    ...
    def GetAttributesList(self):
        ...
        orderedLayersPath = natsorted(os.listdir(self.dir_path + '/input/assets'))
    ...
    def GetItemsPerAttribute(self, attributePath, attribute):
        ...
        files = natsorted(os.listdir(self.dir_path + '/input/assets/' + attributePath))
        itemTombola = [0]
        item = []
        itemPath = files
        for file in files:
    ...
    def CreateNfts(self, nftTotalQuantity, jsonTemplate, attributes, orderedLayersPath, items, itemsPath, itemsTombola, folder_path):
        ...
        for dna in nftsThisRun:
            ...
            nft = Nft(nftsCounterThisRun, nftMetadata['name'], rawAttributes, filteredAttributes,nftMetadata, nftPaths, orderedLayersPath, attributes_count, folder_path, self.dir_path)
    ...
    def CreateExtraJsonFiles(self, i, folder_path):
        ...
        with open(self.dir_path + '/output/nfts/'+ folder_path+'_metadata.json', 'w') as jsonFile:
            json.dump(self.totalMetadata[i], jsonFile, indent = 4)

        with open(self.dir_path + '/output/nfts/'+ folder_path+'_dna.json', 'w') as jsonFile:
    ...
    def CreateTotalNFTInfo(self):
        ...
        with open(self.dir_path + '/output/nfts.json', 'w') as jsonFile:
