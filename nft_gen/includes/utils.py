...
def Args(numberNFTs, testRarities, randomizeOutput, dir_path, task_id, script_uploading):
    parser = OptionParser()
    ...
    parser.add_option('-d', '--dir-path', dest = 'dir_path', help = 'for directory path', metavar = 'str (None or str)', type = 'str')
    parser.add_option('-i', '--task-id', dest = 'task_id', help = 'database task id', metavar = 'int (None or int)', type = 'int')
    parser.add_option('-z', '--script-uploading', dest = 'script_uploading', help = 'database script uploading', metavar = 'int (None or int)', type = 'int')
    ...
    if opts.dir_path:
        dir_path = str(opts.dir_path)
    if opts.task_id:
        task_id = int(opts.task_id)
    if opts.script_uploading: 
        script_uploading = int(opts.script_uploading)
    ...
    return numberNFTs, testRarities, randomizeOutput, dir_path, task_id, script_uploading
